...
export default function SkillsMatrix() {
  const getCategoryIcon = (iconName: string) => {
    switch (iconName) {
      case "Palette": return <Palette className="w-5 h-5 text-brand-400" />;
  ...
  return (
    <section id="habilidades" className="py-24 relative bg-canvas-900/30 border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-xs font-mono text-purple-400">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
          {SKILL_CATEGORIES.map((category, idx) => (
            <div
              key={idx}
              className="p-6 sm:p-8 rounded-3xl glass-panel border border-white/10 hover:border-brand-500/30 transition-all duration-300 flex flex-col gap-6 shadow-xl"
            >
              {/* Category Title */}
              ...
              <div className="space-y-3.5">
                {category.skills.map((skill, sIdx) => (
                  <div
                    key={sIdx}
                    className="p-3.5 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/15 transition-colors space-y-1.5"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <span
                        className={`text-[10px] font-mono px-2 py-0.5 rounded-md border ${
                          skill.badgeColor || "border-white/20 text-gray-300"
                        }`}
                      >
                        {skill.level}
                      </span>
